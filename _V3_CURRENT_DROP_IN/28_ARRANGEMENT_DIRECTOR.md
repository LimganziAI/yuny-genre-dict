...
#              (8-필드 + 핵심 0-2줄 / 불쑥 코멘트는 짧게). 파일 반사적 로드 금지.
...
**ON (자동 발동):** 새 곡 아키텍처(8-필드 前) / "아쉽다·부족·밋밋·한쪽으로만·
...
분기일 때만* 2-3안, 아니면 바로 8-필드. CREATE 베이스 레퍼런스 DNA(누구/어느 권역
...
### §9.1 Level 1 — 아키텍처 (8-필드 *전*, 내부에서 굳힘)
...
분위기 5칸 반영(§6)? / 조합 재점검(§1.4). §2 스타일박스·§9 가사·§12 8-필드을 PD가 부려 씀.
...
→ 선택 → §18 Cascade 동기화 → 풀 8-필드. **수정 2회+ 같은 이슈면 업스트림 진단**
...
### §10.3 효율 가드 (★ 과적화 차단)
- PD 추론 *전부 내부 1패스*. 표면 = 8-필드 + 0-2줄(불쑥 1-2줄). 검증 스탬프·해설 금지(제0원칙·§14).
